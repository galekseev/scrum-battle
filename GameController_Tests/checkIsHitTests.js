...
describe('checkIsHitTests', function () {
  ...
  it('should get health status', function () {
    var ships = gameController.InitializeShipsForTest();
    counter = 1;
    ships.forEach(ship => {
      for (var i = 1; i <= ship.size; i++) {
        ship.addPosition(new position(letters.get(counter), i))
    ...
    assert.equal(ships[0].getHealth(), "Healthy");

    ships[0].hit();

    assert.equal(ships[0].getHealth(), "Hit");

    ships[0].hit();

    assert.equal(ships[0].getHealth(), "Sunk");
